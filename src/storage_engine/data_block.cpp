...
st_e::DataBlock::DataBlock(uint32_t previous_ptr, uint32_t next_ptr, uint32_t creating_transaction_number, uint32_t data_start, uint32_t free_offset,
...
DataBlock::DataBlock(uint32_t previous_ptr, uint32_t next_ptr, uint32_t creating_transaction_number, uint32_t curr_block_ptr)
...
std::vector<char> DataBlock::get_binary_representation() const {
    ...
    std::memcpy(binary_buffer.data() + 6*sizeof(uint32_t), &expire_transaction_number_, sizeof(expire_transaction_number_));
